...
**USB BOOTLoader Setup** 只做一件事：在 **Windows 上** 把普通 U 盘制作成 **可启动的维护 U 盘**（写入引导、PE 与分区布局，并从您指定的 Boot 服务器拉取引导与 ROM 资源）。

**它不负责「在 Windows 里下载整机镜像」**。  
**镜像下载、镜像上传、机房维护等操作**，是在目标电脑 **从这张 U 盘启动之后**，进入维护/PE 环境、由 vDisk 侧能力完成的。请把本工具理解为 **「制盘器」**，而不是镜像客户端本身。

跨 VLAN、跨网段的便利之处在于：只要 **运行本工具的这台电脑** 能访问 Boot 服务器的 `IP:7012`，您就可以在办公网笔记本上把盘做好，再带到机房机器上启动去做镜像下载——**制盘与业务网是否同一网段无关**。
