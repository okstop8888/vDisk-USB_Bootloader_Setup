...
$USBROM = GUICreate("USB BOOTLoader Setup", 454, 210, 228, 114)
...
$UsbStorage = GUICtrlCreateButton("Clean And Writeing", 302, 170, 131, 33)
...
Global $WgetLog = $tempdir & "\wget_download.log"
...
;GUICtrlSetData($Combo3, "Grub4dos")
;GUICtrlSetData($Combo3, "Syslinux")
While 1
	$nMsg = GUIGetMsg()
	Switch $nMsg
...
Func _WgetLogInit($sServerIp)
	FileDelete($WgetLog)
	Local $h = FileOpen($WgetLog, 2)
	If $h = -1 Then Return
	FileWrite($h, "=== USB BOOTLoader wget log ===" & @CRLF)
	FileWrite($h, "Time: " & @YEAR & "-" & @MON & "-" & @MDAY & " " & @HOUR & ":" & @MIN & ":" & @SEC & @CRLF)
	FileWrite($h, "Server: " & StringStripWS($sServerIp, 8) & @CRLF)
	FileWrite($h, "wget messages (append below):" & @CRLF & @CRLF)
	FileClose($h)
EndFunc   ;==>_WgetLogInit


Func _WgetDownload($sServerIp, $sUrlPath, $sOutFile)
	Local $sIp = StringStripWS($sServerIp, 8)
	Local $sUrl = "http://" & $sIp & $sUrlPath
	Local $sExe = $tempdir & "\wget.exe"
	Local $h = FileOpen($WgetLog, 1)
	If $h <> -1 Then
		FileWrite($h, @CRLF & "---- " & $sUrl & " -> " & $sOutFile & " ----" & @CRLF)
		FileClose($h)
	EndIf
	Local $sCmd = '"' & $sExe & '" -t 3 --no-check-certificate -a "' & $WgetLog & '" -O "' & $sOutFile & '" "' & $sUrl & '"'
	Local $iRc = RunWait($sCmd, $tempdir, @SW_HIDE)
	Local $iSz = -1
	If FileExists($sOutFile) Then $iSz = FileGetSize($sOutFile)
	$h = FileOpen($WgetLog, 1)
	If $h <> -1 Then
		FileWrite($h, "---- exit code: " & $iRc & "  saved bytes: " & $iSz & " ----" & @CRLF)
		FileClose($h)
	EndIf
	Return $iRc
EndFunc   ;==>_WgetDownload
...
Func _Clean_And_Write()
	...
	If $ok = 6 Then
		...
		Else
			...
			_WgetLogInit($serverip)
			Local $loader = $tempdir & "\loader"
		
			If _WgetDownload($serverip, ":7012/corex64.gz", $loader & "\corex64.gz") <> 0 Or Not FileExists($loader & "\corex64.gz") Or FileGetSize($loader & "\corex64.gz") < 1 Then
				MsgBox(0, "Download Error", "corex64.gz failed." & @CRLF & "See log:" & @CRLF & $WgetLog)
				Exit
			EndIf
			If _WgetDownload($serverip, ":7012/vmlinuzx64", $loader & "\vmlinuzx64") <> 0 Or Not FileExists($loader & "\vmlinuzx64") Or FileGetSize($loader & "\vmlinuzx64") < 1 Then
				MsgBox(0, "Download Error", "vmlinuzx64 failed." & @CRLF & "See log:" & @CRLF & $WgetLog)
				Exit
			EndIf
			_call("Create DATA ")
			...
			Local $sBootZip = StringReplace($Vol, "/", "\") & "\BOOT\disk.zip"
			Local $sBootGpt = StringReplace($Vol, "/", "\") & "\BOOT\diskgpt.zip"
			If _WgetDownload($serverip, ":7012/rom/disk.zip", $sBootZip) <> 0 Or Not FileExists($sBootZip) Or FileGetSize($sBootZip) < 1 Then
				MsgBox(0, "Download Error", "rom/disk.zip failed." & @CRLF & "See log:" & @CRLF & $WgetLog)
				Exit
			EndIf
			If _WgetDownload($serverip, ":7012/rom/diskgpt.zip", $sBootGpt) <> 0 Or Not FileExists($sBootGpt) Or FileGetSize($sBootGpt) < 1 Then
				MsgBox(0, "Download Error", "rom/diskgpt.zip failed." & @CRLF & "See log:" & @CRLF & $WgetLog)
				Exit
			EndIf
...
Func update()
	...
	Else
		...
		_call("Dwonloading BOOTLoader ...")
		_WgetLogInit($serverip)
		Local $loaderU = $tempdir & "\loader"
		; If _WgetDownload($serverip, ":7012/core.gz", $loaderU & "\core.gz") <> 0 Or Not FileExists($loaderU & "\core.gz") Or FileGetSize($loaderU & "\core.gz") < 1 Then
			; MsgBox(0, "Download Error", "core.gz failed." & @CRLF & "See log:" & @CRLF & $WgetLog)
			; Exit
		; EndIf
		; If _WgetDownload($serverip, ":7012/vmlinuz", $loaderU & "\vmlinuz") <> 0 Or Not FileExists($loaderU & "\vmlinuz") Or FileGetSize($loaderU & "\vmlinuz") < 1 Then
			; MsgBox(0, "Download Error", "vmlinuz failed." & @CRLF & "See log:" & @CRLF & $WgetLog)
			; Exit
		; EndIf
		If _WgetDownload($serverip, ":7012/corex64.gz", $loaderU & "\corex64.gz") <> 0 Or Not FileExists($loaderU & "\corex64.gz") Or FileGetSize($loaderU & "\corex64.gz") < 1 Then
			MsgBox(0, "Download Error", "corex64.gz failed." & @CRLF & "See log:" & @CRLF & $WgetLog)
			Exit
		EndIf
		If _WgetDownload($serverip, ":7012/vmlinuzx64", $loaderU & "\vmlinuzx64") <> 0 Or Not FileExists($loaderU & "\vmlinuzx64") Or FileGetSize($loaderU & "\vmlinuzx64") < 1 Then
			MsgBox(0, "Download Error", "vmlinuzx64 failed." & @CRLF & "See log:" & @CRLF & $WgetLog)
			Exit
		EndIf
		GUICtrlSetData($progressbar1, 50)
		...
		ConsoleWrite(DirCreate($Vol & "/VHD") & @CRLF)
		ConsoleWrite($Vol & "/BOOT" & @CRLF)
		Local $sBootZipU = StringReplace($Vol, "/", "\") & "\BOOT\disk.zip"
		Local $sBootGptU = StringReplace($Vol, "/", "\") & "\BOOT\diskgpt.zip"
		If _WgetDownload($serverip, ":7012/rom/disk.zip", $sBootZipU) <> 0 Or Not FileExists($sBootZipU) Or FileGetSize($sBootZipU) < 1 Then
			MsgBox(0, "Download Error", "rom/disk.zip failed." & @CRLF & "See log:" & @CRLF & $WgetLog)
			Exit
		EndIf
		If _WgetDownload($serverip, ":7012/rom/diskgpt.zip", $sBootGptU) <> 0 Or Not FileExists($sBootGptU) Or FileGetSize($sBootGptU) < 1 Then
			MsgBox(0, "Download Error", "rom/diskgpt.zip failed." & @CRLF & "See log:" & @CRLF & $WgetLog)
			Exit
		EndIf
		GUICtrlSetData($progressbar1, 100)
...
Func _WriteConfig()
	...
	RegWrite("HKEY_CURRENT_USER\Software\vdusbbootloader", "serverip", "REG_SZ", $serverip)
	RegWrite("HKEY_CURRENT_USER\Software\vdusbbootloader", "cmd", "REG_SZ", $cmd)
...
Func _comment($cmdtext)
	...
	$menufiletext[2] = "default 0"
	$menufiletext[3] = "timeout 30"
	$menufiletext[4] = "title BOOTLoader ..."
	$menufiletext[5] = "kernel /loader/vmlinuzx64 loglevel=3  vga=0x37F " & $cmdtext
	$menufiletext[6] = "initrd /loader/corex64.gz"
	$menufiletext[7] = " "
	$menufiletext[8] = "title base vga driver  ..."
	$menufiletext[9] = "kernel /loader/vmlinuzx64 loglevel=3  vga=0x37F nomodeset " & $cmdtext
	$menufiletext[10] = "initrd /loader/corex64.gz"
	_FileWriteFromArray($MenuFile, $menufiletext, 1)
	
	
	Local $menuefix86filetext[10]
	$menuefix86filetext[0] = "set timeout=0"
	;$menuefix86filetext[1] = "set lang=zh_CN"
	$menuefix86filetext[1] = "set pager=0"
	$menuefix86filetext[2] = "insmod biosdisk"
	$menuefix86filetext[3] = "insmod ext2"
	$menuefix86filetext[4] = "insmod part_msdos"
	$menuefix86filetext[5] = "insmod all_video"
	$menuefix86filetext[6] = "menuentry 'UEFI BOOTLoaderIA32 ...' {"
	$menuefix86filetext[7] = "linux	/EFI/vmlinuz loglevel=3   boottype=efi  vga=795   " & $cmdtext
	$menuefix86filetext[8] = "initrd	/EFI/core.gz"
	$menuefix86filetext[9] = "}"
	_FileWriteFromArray($TempEfi & "/EFI/boot/grub.cfg", $menuefix86filetext, 1)
	
	
	Local $menuefix64filetext[14]
	$menuefix64filetext[0] = "set timeout=30"
	;$menuefix64filetext[1] = "set lang=zh_CN"
	$menuefix64filetext[1] = "set pager=0"
	$menuefix64filetext[2] = "insmod biosdisk"
	$menuefix64filetext[3] = "insmod ext2"
	$menuefix64filetext[4] = "insmod part_msdos"
	$menuefix64filetext[5] = "insmod all_video"
	$menuefix64filetext[6] = "menuentry 'UEFI BOOTLoaderX64 ...' {"
	$menuefix64filetext[7] = "linux	/EFI/vmlinuzx64 video=efifb:1920x1080 loglevel=3   boottype=efi  vga=791   " & $cmdtext
	$menuefix64filetext[8] = "initrd	/EFI/corex64.gz"
	$menuefix64filetext[9] = "}"
	$menuefix64filetext[10] = "menuentry 'UEFI BOOTLoaderX64 base vga driver ...' {"
	$menuefix64filetext[11] = "linux	/EFI/vmlinuzx64 video=efifb:1920x1080 loglevel=3   boottype=efi nomodeset vga=791   " & $cmdtext
	$menuefix64filetext[12] = "initrd	/EFI/corex64.gz"
	$menuefix64filetext[13] = "}"
	
	_FileWriteFromArray($TempEfi & "/EFI/boot/grub.cfg", $menuefix64filetext, 1)
